...
export default function HotBreadTab<T extends string>({ tabs, activeTab, setActiveTab }: TabsProps<T>) {
  return (
      ...
      <div className="w-full h-12 relative bg-white flex justify-between items-center px-5">
        ...
        {tabs.map(({ key, label }) => (
          <div
            ...
            <span className={`text-[15px] font-semibold ${activeTab === key ? 'text-primary' : 'text-gray-400'}`}>
              {label}
            </span>
            {activeTab === key && <div className="absolute bottom-0 w-full h-0.5 bg-primary rounded-[99px]" />}
          </div>
        ))}
      </div>
    </div>
