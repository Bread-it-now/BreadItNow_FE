      ...
      <>
        ...
        <div
          className={cn(
            ...
            'w-[88px] min-w-[88px] h-[34px] px-[6px] py-[3px]',
          )}>
          <button onClick={increaseQuantity} className={buttonBasestyle}>
            <Image aria-label="수량 1 증가" src={plusIcon} alt="increase" width={24} height={24} />
          </button>
          <input
            type="number"
            value={quantity}
            ref={ref}
            className={cn('flex items-center justify-center text-center', 'w-6 h-5', 'text-body-s text-gray-900')}
            readOnly
          />
          <button onClick={decreaseQuantity} className={buttonBasestyle}>
            <Image aria-label="수량 1 감소" src={minusIcon} alt="decrease" width={24} height={24} />
          </button>
        </div>
      </>
    );
